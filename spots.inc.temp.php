			...
			<table class="table table-striped table-hover table-condensed" summary="Spots">
				<thead>
					<tr class="head">
						...
						if ($show_comments) {
						?>
						<th class="comments"><a title="<?php echo _('Number of comments'); ?>" href="<?php echo $tplHelper->makeToggleSortUrl('index', 'commentcount', 'DESC'); ?>"><i class="fa fa-comments fa-2x"></i></a></th>
						<?php } 
						?>
						...
						<?php } 
						if ($show_multinzb_checkbox && !count($spots) == 0) { 
						?>
						<th class="multinzb"> 
				...
				?>
				<tr class="noresults"><td colspan="<?php echo $colSpan; ?>"><?php echo _('No results found'); ?></td></tr>
				<?php
				}
				...
				foreach($spots as $spot) {
					...
					<tr class="<?php echo $tplHelper->cat2CssClass($spot); ?> 
					<?php
					if ($spot['hasbeendownloaded']) {
						echo ' downloadedspot';
						$dateTitleText .= _('downloaded on') . ' ' . $tplHelper->formatDate($spot['downloadstamp'], 'force_spotlist');
					} # if
					if ($spot['hasbeenseen']) {
						echo ' seenspot';
						$dateTitleText .= _("opened on") . ' ' . $tplHelper->formatDate($spot['seenstamp'], 'force_spotlist');
					} # if
					if ($spot['moderated'] != 0) {
						echo ' moderatedspot';
					} # if
					?>
					">
					<td class="category"><a href="<?php echo $spot['caturl']; ?>" title="<?php echo sprintf(_("Go to category '%s'"), $spot['catshortdesc']); ?>"><span class="label label-cat label-<?php echo $tplHelper->filter2cat($tplHelper->cat2CssClass($spot)); ?>"><?php echo $spot['catshortdesc']; ?></span></a></td>
					<td class="title <?php echo $newSpotClass; ?>"><a href="<?php echo $spot['spoturl']; ?>" title="<?php echo $spot['title']; ?>" class="spotlink"><?php echo $reportSpam . $rating . $markSpot . $spot['title']; ?></a></td>
					<?php
					if ($show_watchlist_button) {
